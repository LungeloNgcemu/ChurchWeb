...
import '../screens/home/church_screen.dart';
...
//  DateTime _selectedDay = DateTime.now();
  ...
  Future air_time_supabase(List<String> list) async {

    await supabase
        .from('CloudTime')
        .update({'Times': '{${list.join(',')}}'}).match(
        {'id': 1});
  }
  ...
  Future <List<String>>  get_current_list()async {
    try {
      final item = await supabase.from('CloudTime')
          .select('Times')
          .eq('id', 1)
          .single();

      final list = item['Times'];
  ...
  Future<void> update_list_calender(selectedDay) async {
    List<String> list = await get_current_list();
                  ...
                  child: Row(
                    ...
                    children: [
                      MyChips.Chips(
                        ...
                        onSelected: () async {
                          await airTimeList(churchStart.projects['Project']?['Times']);
                          List<String> list = await getCurrentList(setState);
